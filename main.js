...
if (Java.available) {
    ...
    Java.perform(function () {
            ...
            var r0capturePath;
            var processName = "";
            let scriptInjectTime = Java.use("java.lang.System").currentTimeMillis();
            let scriptInjectTimeFormat = Java.use("java.text.SimpleDateFormat").$new("yyyyMMdd_HHmmssSSS").format(Java.use("java.util.Date").$new(scriptInjectTime), Java.use("java.lang.StringBuffer").$new(), Java.use("java.text.FieldPosition").$new(0)).toString();

            function print_arg(addr) {
                try {
                    return "(pointer)   memory dump:\n" + hexdump(addr) + "\n";
                } catch (e) {
                    console.log("print_arg failed:", e);
                    return addr + "\n";
                }
            }
            ...
            function writeFile(file, content, append) {
                ...
                if (!file.exists()) {
                    file = createNewFile(file.getPath());
                }
                try {
                    let fos = Java.use("java.io.FileOutputStream").$new(file, append);
                    fos.write(content);
                    fos.close();
                    return true;
                } catch (e) {
            ...
            function createNewFile(path) {
                ...
                let file = Java.use("java.io.File").$new(path);
                try {
                    if (file.exists()) {
                        return file;
                    }
            ...
            function printLog(message, data) {
                let msg = Java.use("java.lang.StringBuilder").$new("\n========START===========\n");
                msg = msg.append("\n[+] Function: " + message['function']);
                msg = msg.append("\n[+] Ssl_session_id: " + message['ssl_session_id']);
                ...
                if (!r0capturePath) {
                    ...
                    if (!app) {
                        try {
                            ...
                            console.log("get app failed:", e);
                        }
                    }
                    console.log("app:", app);
                    if (!app) {
                        r0capturePath = Java.use("android.os.Environment").getExternalStorageDirectory().getPath().concat("/Android/obj/r0capture/");
                    } else {
                        r0capturePath = app.getExternalCacheDir().getPath().concat("/r0capture/");
                        try {
                            processName = Java.use("android.app.ActivityThread").currentProcessName();
                        } catch (e) {
                            console.log("get process name failed:",e);
                        }
                    }
                }
                console.log("r0capturePath:", r0capturePath);
                console.log("processName:",processName);
                let logPath = r0capturePath.concat("/")
                    .concat(processName)
                    .concat("/")
                    .concat(Java.use("java.lang.Thread").currentThread().getName())
                    .concat("_")
                    .concat(scriptInjectTimeFormat)
                    .concat(".log");
                let file = createNewFile(logPath);
                try {
                    if (!writeFile(file, msg.toString().getBytes(), true)) {
                        console.log("write log failed");
                        File.writeAllText(logPath, msg.toString());
                    }
                } catch (e) {
                    console.log(e);
                    File.writeAllText(logPath, msg.toString());
                    ...
                    Interceptor.attach(addresses["SSL_read"],
                        {
                            ...
                            onLeave: function (retval) {
                                ...
                                printLog(this.message, Memory.readByteArray(this.buf, retval));
                            }
                        });
                    ...
                    Interceptor.attach(addresses["SSL_write"],
                        {
                            onEnter: function (args) {
                                ...
                                printLog(message, Memory.readByteArray(args[1], parseInt(args[2])));
                            },
                            onLeave: function (retval) {
                            }
                        });
                } catch (e) {
                    console.log("hook SSL failed:", e);
                }
            })();
            ...
            Java.use("java.security.KeyStore$PrivateKeyEntry").getPrivateKey.implementation = function () {
                ...
                printLog(message, Memory.readByteArray(data, 1))
                return result;
            }
            ...
            Java.use("java.net.SocketOutputStream").socketWrite0.overload('java.io.FileDescriptor', '[B', 'int', 'int').implementation = function (fd, bytearry, offset, byteCount) {
                ...
                message["src_addr"] = this.socket.value.getLocalAddress().toString();
                message["src_port"] = parseInt(this.socket.value.getLocalPort().toString());
                message["dst_addr"] = this.socket.value.getRemoteSocketAddress().toString();
                ...
                printLog(message, Memory.readByteArray(ptr, byteCount))
                return result;
            }
            Java.use("java.net.SocketInputStream").socketRead0.overload('java.io.FileDescriptor', '[B', 'int', 'int', 'int').implementation = function (fd, bytearry, offset, byteCount, timeout) {
                ...
                message["ssl_session_id"] = "";
                message["src_addr"] = this.socket.value.getRemoteSocketAddress().toString();
                message["src_port"] = parseInt(this.socket.value.getRemoteSocketAddress().toString().split(":").pop());
                message["dst_addr"] = this.socket.value.getLocalAddress().toString();
                ...
                if (result > 0) {
                    ...
                    for (var i = 0; i < result; ++i)
                        Memory.writeS8(ptr.add(i), bytearry[offset + i]);
                    printLog(message, Memory.readByteArray(ptr, result))
                }
                return result;
            }
